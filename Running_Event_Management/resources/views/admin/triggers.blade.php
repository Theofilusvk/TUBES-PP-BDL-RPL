...
@section('title', 'Database Triggers | Pelari Kalcer')
...
<main class="flex-1 flex flex-col min-w-0 overflow-hidden bg-background-dark">
    <header class="h-16 flex items-center justify-between px-8 border-b border-border-dark bg-black/50 backdrop-blur-md z-10">
        ...
        <div class="flex items-center gap-3">
            <div class="relative">
                <form method="GET" action="{{ route('admin.triggers') }}">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">search</span>
                    <input name="search" value="{{ request('search') }}" class="w-64 h-8 bg-card-dark border border-border-dark rounded text-[10px] pl-9 focus:ring-1 focus:ring-primary focus:border-primary text-slate-300 placeholder:text-slate-600 uppercase tracking-widest font-bold" placeholder="Query Object Logs..." type="text"/>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-border-dark">
                        ...
                        <tr class="hover:bg-white/[0.02] transition-colors group">
                            <td class="px-6 py-4">
                                <p class="text-xs font-bold text-white terminal-text">{{ $log->DetailAktivitas }}</p>
                                <p class="text-[9px] text-slate-500 mt-0.5 uppercase tracking-tighter font-mono">User: {{ $log->Username }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-[9px] font-black text-primary border border-primary/30 px-2 py-0.5 uppercase">{{ $log->TipeAktivitas }}</span>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="text-[10px] text-slate-500 uppercase font-medium">{{ \Carbon\Carbon::parse($log->WaktuLog)->diffForHumans() }}</span>
                            </td>
