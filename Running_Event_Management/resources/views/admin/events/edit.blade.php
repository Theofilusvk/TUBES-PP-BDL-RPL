...
@section('title', 'Edit Event | ' . $event->NamaEvent)
...
<main class="flex-1 flex flex-col min-w-0 overflow-hidden bg-background-dark">
    ...
    <header class="h-24 border-b border-border-dark flex items-center justify-between px-10 bg-background-dark/80 backdrop-blur-xl z-10 shrink-0">
        <div class="flex items-center gap-6">
            <a href="{{ route('admin.events.show', $event->EventID) }}" class="w-10 h-10 rounded-xl bg-surface-dark border border-border-dark flex items-center justify-center text-slate-400 hover:text-white hover:border-primary transition-all group">
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-10">
        <div class="max-w-4xl mx-auto">
            ...
            <form id="edit-event-form" action="{{ route('admin.events.update', $event->EventID) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <section class="bg-card-dark rounded-2xl border border-border-dark p-8">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <input type="text" name="NamaEvent" value="{{ old('NamaEvent', $event->NamaEvent) }}" required class="w-full bg-surface-dark border border-border-dark rounded-xl px-4 py-3 text-white font-bold focus:ring-1 focus:ring-primary focus:border-primary transition-all placeholder:text-slate-600">
                        </div>
                        
                        <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Description</label>
                            <textarea name="DeskripsiEvent" rows="5" class="w-full bg-surface-dark border border-border-dark rounded-xl px-4 py-3 text-white font-medium focus:ring-1 focus:ring-primary focus:border-primary transition-all placeholder:text-slate-600 resize-none">{{ old('DeskripsiEvent', $event->DeskripsiEvent) }}</textarea>
                        </div>

                         <div>
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Location / Terrain</label>
                            <input type="text" name="Location" value="{{ old('Location', $event->LokasiEvent) }}" required class="w-full bg-surface-dark border border-border-dark rounded-xl px-4 py-3 text-white font-bold focus:ring-1 focus:ring-primary focus:border-primary transition-all placeholder:text-slate-600">
                        </div>
                    </div>
                </section>
                            ...
                            <div class="flex items-center gap-4">
                                @if($event->GambarEvent)
                                    <div class="w-16 h-16 rounded-lg overflow-hidden border border-border-dark shrink-0">
                                        <img src="{{ asset($event->GambarEvent) }}" class="w-full h-full object-cover">
                                    </div>
                                @endif
