...
@section('title', 'Notification Center | Pelari Kalcer')
...
<header class="p-10 pb-6">
    <div class="flex flex-wrap items-end justify-between gap-6">
        ...
        <div>
             <a href="{{ route('admin.users') }}" class="px-6 py-2.5 bg-surface-dark text-slate-300 border border-border-dark rounded-xl text-sm font-bold hover:text-white hover:border-primary transition-all flex items-center gap-2">
...
<section class="px-10 py-4 grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1">
        <div class="bg-card-dark border border-border-dark rounded-xl p-6 relative overflow-hidden">
            ...
            <form action="{{ route('admin.notifications.broadcast') }}" method="POST" class="space-y-5">
                @csrf
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select name="EventID" class="w-full bg-surface-dark border border-border-dark rounded-xl px-4 py-3 text-white font-bold focus:ring-1 focus:ring-primary focus:border-primary transition-all appearance-none text-sm">
                            ...
                            @foreach($events as $event)
                                <option value="{{ $event->EventID }}">{{ $event->NamaEvent }}</option>
                            @endforeach
                        </select>
             ...
             <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-border-dark">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        <td class="px-6 py-4">
                            <span class="text-xs font-mono font-bold text-slate-400">{{ \Carbon\Carbon::parse($log->SentTime ?? $log->WaktuLog ?? now())->format('d M H:i') }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm font-bold text-white">{{ $log->JudulNotifikasi }}</p>
                            <p class="text-[10px] text-slate-500 line-clamp-1">{{ Str::limit($log->IsiNotifikasi, 50) }}</p>
                        </td>
                         <td class="px-6 py-4">
                            <span class="text-[10px] font-bold text-slate-300 bg-surface-dark border border-border-dark px-2 py-1 rounded uppercase tracking-wider">{{ $log->TipeNotifikasi }}</span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                <span class="material-symbols-outlined text-emerald-500 text-sm">check_circle</span>
                                <span class="text-xs font-bold text-emerald-500">{{ $log->RecipientCount }} Sent</span>
                            </div>
                        </td>
                    </tr>
                    @empty
