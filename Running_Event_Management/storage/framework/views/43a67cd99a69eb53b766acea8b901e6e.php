...
<?php $__env->startSection('title', 'Financial Reports | Pelari Kalcer'); ?>
...
<main class="flex-1 flex flex-col overflow-hidden bg-background-dark">
    <header class="sticky top-0 z-10 flex items-center justify-between px-10 py-6 bg-background-dark/80 backdrop-blur-xl border-b border-border-dark">
        ...
        <div class="flex items-center gap-6">
            <form method="GET" action="<?php echo e(route('admin.financial')); ?>" class="relative">
                <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 text-xl">search</span>
                <input name="search" value="<?php echo e(request('search')); ?>" class="w-80 pl-12 pr-6 py-2.5 bg-card-dark border border-border-dark rounded-full text-sm focus:ring-1 focus:ring-primary focus:border-primary transition-all text-white placeholder-gray-600" placeholder="Search hash, runner, or event..." type="text"/>
            </form>
        </div>
    </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <!-- ... (Stats content omitted for brevity, keeping existing) ... -->
             <div class="bg-card-dark p-6 rounded-2xl border border-border-dark hover:border-primary/30 transition-all group">
                ...
                <p class="text-2xl font-black text-white tracking-tighter italic">Rp <?php echo e(number_format($totalRevenue, 0, ',', '.')); ?></p>
            </div>
             <!-- ... other stats ... -->
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-border-dark">
                        ...
                        <tr class="hover:bg-white/[0.02] transition-colors group">
                            <td class="px-8 py-5 text-sm font-black text-gray-300 group-hover:text-primary">#PK-<?php echo e($txn->PembayaranID); ?></td>
                            <td class="px-8 py-5">
                                <div class="flex items-center gap-3">
                                    <div class="size-8 rounded-lg bg-primary/10 flex items-center justify-center border border-primary/20">
                                        <span class="text-primary font-black text-[10px]"><?php echo e(substr($txn->RunnerName ?? 'Unknown', 0, 2)); ?></span>
                                    </div>
                                    <span class="text-sm font-bold text-gray-200"><?php echo e($txn->RunnerName ?? $txn->Username); ?></span>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-sm text-gray-400 font-medium italic">
                                <?php echo e($txn->NamaEvent ?? $txn->NamaKategori ?? 'Event Unknown'); ?>

                            </td>
                            <td class="px-8 py-5 text-sm font-black text-white italic">Rp <?php echo e(number_format($txn->NominalBayar, 0, ',', '.')); ?></td>
                            <td class="px-8 py-5 text-[10px] font-bold text-gray-500 uppercase"><?php echo e(\Carbon\Carbon::parse($txn->TanggalBayar)->format('M d, H:i')); ?></td>
                            <td class="px-8 py-5">
                                <span class="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-[10px] font-black rounded-full border border-emerald-500/20"><?php echo e($txn->StatusPembayaran); ?></span>
                            </td>
                             <td class="px-8 py-5 text-right">
                                <?php if($txn->StatusPembayaran === 'Ditolak'): ?>
                                ...
                                <?php elseif($txn->StatusPembayaran === 'Lunas'): ?>
                                ...
                                <?php else: ?>
                                    <button onclick="openModal('<?php echo e($txn->PembayaranID); ?>', '<?php echo e($txn->BuktiPembayaran ?? ''); ?>')" class="bg-surface-dark border border-border-dark hover:border-primary text-slate-300 hover:text-white px-3 py-1 rounded-lg text-[10px] uppercase font-bold tracking-widest transition-all">
                                        Inspect
                                    </button>
                                <?php endif; ?>
                            </td>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Kuliah\Semester 3\Rekayasa Perangkat Lunak\TUBES-PP-BDL-RPL\Running_Event_Management\resources\views/admin/financial.blade.php ENDPATH**/ ?>