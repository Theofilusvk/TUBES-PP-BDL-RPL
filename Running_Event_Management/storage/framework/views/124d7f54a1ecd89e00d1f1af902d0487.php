...
<head>
    ...
    <title>Kalcer Run - <?php echo $__env->yieldContent('title', 'Dashboard'); ?></title>
...
<body class="bg-background-light dark:bg-background-dark text-gray-900 dark:text-gray-100 font-sans h-screen flex overflow-hidden selection:bg-primary selection:text-white transition-colors duration-300" x-data="{ sidebarOpen: false }">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 w-64 bg-primary dark:bg-slate-900 flex flex-col justify-between border-r border-white/10 dark:border-gray-800 transition-transform duration-300 z-30 shadow-xl lg:static lg:translate-x-0">
        <div>
            ...
            <nav class="mt-6 px-3 space-y-1.5 overflow-y-auto max-h-[calc(100vh-280px)]">
                <a class="group flex items-center px-3 py-2.5 text-blue-100 hover:text-white hover:bg-white/10 rounded-lg transition-colors <?php echo e(request()->routeIs('dashboard') ? 'bg-white/10 text-white shadow-inner' : ''); ?>" href="<?php echo e(route('dashboard')); ?>">
                ...
                <div class="relative" x-data="{ open: <?php echo e(request()->routeIs('dashboard.events*') ? 'true' : 'false'); ?> }">
                    ...
                    <div x-show="open" x-collapse class="block pl-3 pr-2 py-2 space-y-1 border-l-2 border-accent/30 ml-4 lg:ml-6 my-1">
                        <a href="<?php echo e(route('dashboard.events', ['filter' => 'upcoming'])); ?>" class="block px-2 py-1.5 text-sm <?php echo e(request()->fullUrlIs('*filter=upcoming*') ? 'text-white bg-accent/20 rounded font-medium' : 'text-blue-200 hover:text-white hover:bg-white/5 rounded transition-colors'); ?>">
                            Upcoming
                        </a>
                        <a href="<?php echo e(route('dashboard.events', ['filter' => 'past'])); ?>" class="block px-2 py-1.5 text-sm <?php echo e(request()->fullUrlIs('*filter=past*') ? 'text-white bg-accent/20 rounded font-medium' : 'text-blue-200 hover:text-white hover:bg-white/5 rounded transition-colors'); ?>">
                            Past Events
                        </a>
                        <a href="<?php echo e(route('dashboard.events', ['filter' => 'my_events'])); ?>" class="block px-2 py-1.5 text-sm <?php echo e(request()->fullUrlIs('*filter=my_events*') ? 'text-white bg-accent/20 rounded font-medium' : 'text-blue-200 hover:text-white hover:bg-white/5 rounded transition-colors'); ?>">
                            My Events
                        </a>
                    </div>
                </div>
                ...
                <a class="group flex items-center px-3 py-2.5 text-blue-100 hover:text-white hover:bg-white/10 rounded-lg transition-colors <?php echo e(request()->routeIs('dashboard.results') ? 'bg-white/10 text-white shadow-inner' : ''); ?>" href="<?php echo e(route('dashboard.results')); ?>">
                ...
                <a class="group flex items-center px-3 py-2.5 text-blue-100 hover:text-white hover:bg-white/10 rounded-lg transition-colors <?php echo e(request()->routeIs('dashboard.leaderboards') ? 'bg-white/10 text-white shadow-inner' : ''); ?>" href="<?php echo e(route('dashboard.leaderboards')); ?>">
        ...
        <div class="p-4 border-t border-white/10 dark:border-gray-800 bg-black/10">
            <a class="group flex items-center px-3 py-2 mb-3 text-blue-100 hover:text-white hover:bg-white/10 rounded-lg transition-colors <?php echo e(request()->routeIs('dashboard.settings') ? 'bg-white/10 text-white shadow-inner' : ''); ?>" href="<?php echo e(route('dashboard.settings')); ?>">
            ...
            <!-- User Info (Visible on Mobile Sidebar too) -->
             <div class="flex items-center gap-3 px-3 py-2 border-t border-white/10 pt-4 mt-2">
                <div class="h-8 w-8 rounded-full bg-gradient-to-tr from-primary to-accent p-[1px]">
                     <img alt="Profile" class="h-full w-full rounded-full object-cover" src="<?php echo e(Auth::user()->Gambar ?? 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->NamaLengkap ?? 'User') . '&background=random'); ?>"/>
                </div>
                <div class="flex-1 min-w-0">
                    <div class="text-sm font-bold text-white truncate"><?php echo e(Auth::user()->NamaLengkap ?? 'User'); ?></div>
                    <div class="text-xs text-blue-200 truncate"><?php echo e(Auth::user()->Email); ?></div>
                </div>
        ...
        <header class="h-20 bg-white dark:bg-card-dark border-b border-gray-200 dark:border-gray-800 flex items-center justify-between px-4 lg:px-8 z-10 shadow-sm shrink-0">
            <div class="flex items-center gap-4">
                ...
                <nav class="hidden md:flex items-center text-sm font-medium text-gray-500 dark:text-gray-400">
                    <a class="hover:text-primary transition-colors" href="<?php echo e(route('dashboard')); ?>">Dashboard</a>
                    <span class="material-icons text-base mx-2">chevron_right</span>
                    <span class="text-gray-900 dark:text-white font-semibold"><?php echo $__env->yieldContent('header_title', 'Events Hub'); ?></span>
                </nav>
    ...
    <script>
        ...
        window.userNotifications = <?php echo json_encode($notifications ?? [], 15, 512) ?>;
        window.userUnreadCount = <?php echo json_encode($unreadCount ?? 0, 15, 512) ?>;
...
<?php /**PATH C:\Kuliah\Semester 3\Rekayasa Perangkat Lunak\TUBES-PP-BDL-RPL\Running_Event_Management\resources\views/layouts/dashboard.blade.php ENDPATH**/ ?>